  ...
  return (
    <>
      ...
      <div className="lg:hidden w-full bg-gray-50 dark:bg-neutral-800 border-b-[1px] h-14 z-50 flex items-center px-4 justify-between fixed ">
        ...
        >
          <div
            className={`w-6 h-[2px]  dark:bg-white bg-black transition-all ${isOpen ? "rotate-45 translate-y-2" : ""}`}
          ></div>
          <div
            className={`w-6 h-[2px]  dark:bg-white bg-black transition-all ${isOpen ? "opacity-0" : "opacity-100"}`}
          ></div>
          <div
            className={`w-6 h-[2px]  dark:bg-white bg-black transition-all ${isOpen ? "-rotate-45 -translate-y-2" : ""}`}
          ></div>
      ...
      {/* Mobile Sidebar Menu */}
      <div
        className={`lg:hidden fixed top-14 left-0 w-full bg-gray-100 dark:bg-neutral-900 z-40 transition-all duration-300 ease-in-out ${isOpen ? "h-[calc(100svh-4rem)]" : "h-0 overflow-hidden"}`}
      >
        <div className="py-4">
