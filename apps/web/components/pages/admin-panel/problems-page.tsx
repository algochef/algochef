    ...
    <div className="flex flex-1 flex-col my-2 p-4">
      <div className="flex flex-col-reverse md:flex-row justify-between">
        <div>
          ...
          <h4 className="text-sm lg:text-base">
            Manage and organize coding problems for your platform.
          </h4>
        </div>
        <div>
          <AddProblemModal />
      ...
      {problems.map((problem) => {
        return (
          <div
            key={problem.slug}
            className="flex w-full space-x-3 rounded-md border-2 border-gray-500/10 justify-between my-2 p-4 "
          >
              ...
              <div className="flex flex-col space-y-1">
                <span>
                  <span className="flex space-x-1">
                    ...
                    <DifficultyLabel
                      difficulty={
                        problem.difficultyNumeric || problem.difficultyCategory
                      }
                    />
                  </span>
                  <span className="flex space-x-1">
                    ...
                    {problem.companyTags &&
                      problem.companyTags.map((tag) => {
                        return (
                          <h2
                            key={tag.slug}
                            className="text-sm tracking-tighter  bg-gradient-to-t from-gray-300/50 to-gray-100/50 rounded-md px-2 py-0.5 text-gray-600 w-fit shadow border-[1px] h-fit"
                          >
                            <span className="flex space-x-1">
                    ...
                    {problem.tags &&
                      problem.tags.map((tag) => {
                        return (
                          <p
                            key={tag.slug}
                            className="text-sm tracking-tighter border-[1px] bg-gradient-to-b from-blue-700/10 to-blue-50 border-blue-400/30 rounded-md px-2 w-fit h-fit shadow"
                          >
                            {tag.name}
                          </p>
                        );
            ...
            <div className="flex space-x-2 items-center">
              <Link
                href={problem.url}
                className="flex space-x-1 items-center text-xs px-2 py-1 bg-gray-950 w-fit text-gray-50 rounded h-fit"
              >
