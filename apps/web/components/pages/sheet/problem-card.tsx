  ...
  return (
    <div className="flex flex-col border-[1px] rounded border-gray-100 dark:border-neutral-800 p-3  w-full justify-center">
      <div className="flex space-x-2 items-center">
        ...
        <h2 className="md:text-md tracking-tight font-semibold">
          {problem.title}
        </h2>
        <DifficultyLabel
          difficulty={problem.difficultyCategory || problem.difficultyNumeric}
        />
      </div>
        ...
        {problem &&
          ...
          problem.companyTags.map((tag, idx) => {
            if (idx > 3 || !problem || !problem.companyTags) return;
              ...
              return (
                <CustomLabel
                  title={`+${(problem.companyTags.length - idx).toString()}`}
                  key={tag.slug}
                />
              );
            }
