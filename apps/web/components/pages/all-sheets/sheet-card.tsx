  ...
  return (
    ...
    >
      <div className="relative z-10">
        <h2 className="tracking-tight font-semibold text-lg md:text-xl text-gray-50">
          {sheet.title}
        </h2>
        <h6 className="text-xs md:text-sm tracking-tighter text-gray-200/80">
          {sheet.description}
        </h6>
      </div>

      <div
        className={`-rotate-45 absolute right-2 bottom-2 transform transition-all duration-200 ease-in-out z-0 w-[80px] h-[80px]  sm:w-[100px] sm:h-[100px] md:w-[120px] md:h-[120px] ${isHovered ? "scale-150" : ""}`}
      >
        <Image
          src={COLORS[sheet.theme].icon}
          alt="icon"
          width={120}
          height={120}
        />
