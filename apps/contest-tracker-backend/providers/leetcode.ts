...
const getPastContests = async (page: number): Promise<any[]> => {
  ...
  const res = await fetch("https://leetcode.com/graphql/", {
    ...
    body: JSON.stringify({
      query:
        "query pastContests($pageNo: Int, $numPerPage: Int) { pastContests(pageNo: $pageNo, numPerPage: $numPerPage) { pageNum currentPage totalNum numPerPage data { title titleSlug startTime duration } } }",
      variables: {
        pageNo: page,
      },
