...
export const codechefUserDetails = async (handle: string) => {
    try {
        ...
        const maxRating = parseNumber($('div.rating-header').eq(0).find('small').text().trim()) || 0;
        const rank = $('span.rating').text().trim();
        const totalContests = parseInt($('div.contest-participated-count>b').text().trim()) || 0;
        const totalSolved = parseNumber($('section.problems-solved').find('h3').eq(3).text().trim()) || 0;
